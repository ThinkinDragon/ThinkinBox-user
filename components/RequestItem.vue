<template>
    <NuxtLink :to="request.status === 'COMPLETED' || request.status === 'SCHEDULED' ? `/booking/${request.id}-detail` : `/booking/${request.id}`" class="inline-flex flex-col items-start justify-center w-full my-2">
      <UCard >
        <template #header>

        <div class="flex flex-row items-end justify-start w-full">
          <div class="inline-flex w-16 flex-col items-start justify-start">
          ...
          <div class="inline-flex w-full flex-col items-start justify-start gap-1">
                <div class="flex flex-row gap-2">
                  <p class="text-sm text-gray-900 dark:text-gray-200">{{ format(new Date(request.created_at), "MMMM dd, yyyy ") }}</p>
                  <p class="text-sm text-gray-900 dark:text-gray-200">{{ format(new Date(request.created_at), "hh:mm aa") }}</p>
                </div>
            <p class="w-full text-base font-bold text-gray-900 dark:text-gray-50 capitalize">{{ $_api_(request.service_type.name) }} | {{ $__("Id") }} : #{{ request.booking_id }}</p>
            <p class="w-full text-xs font-medium leading-none text-gray-500 dark:text-gray-300"></p>
          </div>
        </div>
        </template>
        <RequestStatus :status="request.status" />
        <div class="flex flex-col items-start justify-start">
          <!-- <div v-if="request?.status === 'COMPLETED'" class="inline-flex items-center justify-start pr-12 w-full">
            <div class="inline-flex flex-row items-start justify-center flex-1 my-3 space-x-2">
              <div class="flex items-center justify-center h-full p-2 ">
                <svg class="flex-1 w-4 h-4 rounded-lg" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div v-if="request?.status === 'SCHEDULED'" class="inline-flex items-center justify-start pr-12 w-full">
            <div class="inline-flex flex-row items-start justify-center flex-1 my-3 space-x-2">
              <div class="flex items-center justify-center w-1/5 h-full p-2 ">
          ...
          <div v-else class="inline-flex items-center justify-start pr-12 w-full">
            <div class="inline-flex flex-row items-start justify-center flex-1 my-3 space-x-2">
              ...
              <div class="flex flex-row items-center justify-around w-full h-full">
                ...
                <p class="inline-flex text-xs font-bold leading-none text-gray-500">{{ $__("Created at") }}</p>
              </div>
            </div>
          </div> -->
          <div class="inline-flex items-center justify-between w-full space-x-4">
            <div class="inline-flex flex-col items-start justify-start">
              <p class="text-xs font-bold leading-none text-gray-500">{{ $__("Pickup location") }}</p>
              <p class="text-xs font-bold text-center">{{ request?.s_address }}</p>
            </div>
            <div class="hidden  items-center justify-between rounded-xl bg-primary px-5 py-2.5 shadow">
          ...
          <div class="inline-flex flex-col items-start justify-start">
            <p class="text-xs font-bold leading-none text-gray-500">{{ $__("Drop location") }}</p>
            <p class="text-xs font-bold text-center">{{ request?.s_address }}</p>
          </div>
        </div>
      </UCard>
