<template>
  <div>
    <NuxtLayout name="single">
      <div class="flex flex-col items-center justify-center w-full p-2 space-y-8 rounded-lg">
        <LoadingData :data="booking">
          ...
          <div v-show="booking?.payment" class="inline-flex flex-col items-center justify-center w-full space-y-3">
            ...
            <div v-if="booking?.status === 'SCHEDULED'" class="flex flex-col items-start justify-start w-full">
              <button  @click="cancel(booking?.id, 'Schedule request cancel')" class="flex items-center justify-center w-full h-full px-2 py-3 bg-accent rounded-lg">
...
const { useBookings, accept, reject, updateBook, rating, cancel } = useBooking();
const store = useBookings();
const { booking, flow } = storeToRefs(store);
