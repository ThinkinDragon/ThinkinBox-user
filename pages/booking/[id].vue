      ...
      <div class="container mx-auto overflow-y-scroll mb-4">
        ...
        <client-only>
          ...
          <LoadingData :data="booking">
              <div v-if="booking?.status === 'DROPPED'" class="inline-flex flex-col items-center justify-center w-full space-y-3">
                <div class="flex flex-col items-center justify-center w-full space-y-3">
                  ...
                  <div class="flex w-full flex-1 items-start justify-start px-2.5">
                    ...
                    <button v-if="booking.payment_mode == 'CASH'" class="flex items-center justify-center flex-1 px-5 py-3 rounded-xl bg-primary">
                      <p class="text-base font-semibold text-white dark:text-gray-900">{{$__("Done")}}</p>
                    </button>
                    
                    <button v-else @click="store.makePayment(route?.params?.id, booking.payment_mode, 0)" class="flex items-center justify-center flex-1 px-5 py-3 rounded-xl bg-primary">
              ...
              <div v-else-if="booking?.paid === 1 && booking?.user_rated == 0" class="flex flex-col items-center justify-center w-full mt-2 space-x-4 space-y-4">
              ...
              <div v-else class="flex flex-col flex-1 w-full space-y-3">
                <template v-if="booking?.status == 'SEARCHING'">
                    ...
                    <Vue3Lottie class="flex items-center justify-center h-60" :animationData="DogJSON" :height="100" :width="200" />
                  ...
                  <div class="flex flex-row items-end justify-start w-full pb-4 border-b-2">
                    <div class="inline-flex w-16 flex-col items-start justify-start space-y-1.5">
                      <div class="flex flex-col items-center justify-center w-12 h-12">
                        <img class="h-full rounded-full" :src="config.public.BASE_IMAGE_URL + booking?.service_type?.image" />
                      </div>
                    </div>
                    <div class="inline-flex w-60 flex-col items-start justify-start space-y-1.5">
                      ...
                      <p class="w-full text-xs font-medium leading-none text-gray-500">{{ $__("Reference Code") }} : #{{ booking?.booking_id }}</p>
                    </div>
                  </div>
                  ...
                  <div v-show="booking?.provider != undefined" class="flex flex-row space-x-2.5 items-center justify-evenly w-full">
                    ...
                    <div class="inline-flex flex-col items-start justify-start">
                      <div class="flex flex-col items-start justify-start space-y-1">
                        ...
                        <br>
                        <p class="w-full text-sm font-medium text-gray-900 dark:text-gray-200">OTP : {{ booking?.otp }}</p>
                    ...
                    <div v-if="booking?.status === 'SCHEDULED'" class="inline-flex items-center justify-start pr-12">
                      <div class="inline-flex flex-row items-start justify-center flex-1 my-3 space-x-2">
                        ...
                        <div class="inline-flex flex-col items-start justify-end h-full">
                          <p class="text-sm text-gray-900 dark:text-gray-200">{{ format(new Date(booking.schedule_at), "MMMM dd, yyyy hh:mm aa") }}</p>
                ...
                <div v-show="booking?.status != 'PICKEDUP' && booking?.status != 'COMPLETED'" class="inline-flex items-center justify-center w-full mt-2 space-x-4">
                  <UButton size="lg" color="red" v-show="!showReason" @click="showReason = !showReason" class="flex items-center justify-center w-full h-full px-2 py-3 ">
        ...
        <UModal v-model="showReason" class="z-[99999999] p-2">
          ...
          <div class="inline-flex items-center justify-center w-full mt-2 space-x-4">
            <UButton size="lg" @click="showReason = !showReason" class="flex items-center justify-center px-2 w-1/2 py-3 bg-primary rounded-lg">
              <p class="text-base font-semibold text-white dark:text-gray-900">{{ $__("back") }}</p>
            </UButton>
            <UButton size="lg" color="red" @click="cancel(booking?.id, formm?.reason ?? 'user cancel request')" class="flex items-center justify-center w-1/2 px-2 py-3">
              <p class="text-base font-semibold text-white dark:text-gray-900">{{$__("Cancel")}}</p>
            </UButton>
          </div>
            </template>
