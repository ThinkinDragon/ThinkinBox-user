<template>
  <div>
    <NuxtLayout name="single">
      <div
        class="inline-flex flex-col items-center justify-end w-full pt-4 pb-6 pl-5 pr-4 space-y-6 bg-gray-50 dark:bg-gray-900">
        ...
        <div class="relative flex flex-col w-full p-5 space-y-4">
          <UCard v-for="(item, index) in languages" :key="index">
            <div @click="setLanguage(item)" class="flex h-14 items-center justify-between space-x-2 px-2.5 py-1.5 shadow">
              <p class="text-base text-start line-clamp-1">{{ $__(item) }}</p>
              <div class="flex items-center justify-start space-x-1 min-w-6">
                <svg v-if="language != item" class="h-5 rounded-lg min-w-5" width="16" height="17" viewBox="0 0 16 17"
