  ...
  <div>
    <NuxtLayout name="single">
      ...
      <div v-if="service" class="flex flex-col">
        <Title>{{ title }}</Title>
        <SubServiceItem v-if="service?.children_recursive?.length > 0" v-for="(item, index) in service?.children_recursive" :key="index" :service="item" class="m-2" />
        <div v-else>
          <div class="grid max-w-4xl grid-cols-1 mx-auto">
            <div class="relative flex flex-col-reverse col-start-1 row-start-1 p-3 bg-gradient-to-t from-black/75 via-black/0">
                      <button class="inline-flex items-center justify-center text-white dark:text-gray-900 rounded-md bg-blue-500 px-2 py-0.5 w-fit shadow" @click="showDesc = !showDesc">
              ...
              <h1 class="mt-1 text-lg font-semibold text-white dark:text-gray-900 sm:text-slate-900 md:text-2xl dark:sm:text-white dark:text-gray-900">{{ $_api_(service.name) }}</h1>
            ...
            <div class="grid col-start-1 col-end-3 row-start-1 gap-4">
              <img :src="config.public.BASE_IMAGE_URL + service.image" alt="" class="object-cover w-full max-h-60" loading="lazy" />
            </div>
          ...
          <!-- Coupon Cell -->
          <van-coupon-cell :coupons="coupons" :chosen-coupon="chosenCoupon" @click="showList = true" :currency="`${user?.currency}`" />
          <!-- Coupon List -->
            ...
            <form @submit.prevent="listProvider" enctype="multipart/form-data" class="inline-flex flex-col items-center justify-center w-full px-1 py-4 space-y-5 bg-gray-50 dark:bg-gray-900">
              <div class="flex flex-col items-end justify-start space-y-1.5">
                <div class="hidden 6xl:inline-flex items-center justify-center space-x-2.5 rounded-lg bg-blue-500 p-2">
                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M17.707 9.293C17.8945 9.48053 17.9998 9.73484 17.9998 10C17.9998 10.2652 17.8945 10.5195 17.707 10.707L10.707 17.707C10.5195 17.8945 10.2652 17.9998 10 17.9998C9.73484 17.9998 9.48053 17.8945 9.293 17.707L2.293 10.707C2.2 10.6143 2.12624 10.5041 2.07596 10.3827C2.02568 10.2614 1.99986 10.1313 2 10V5C2 4.20435 2.31607 3.44129 2.87868 2.87868C3.44129 2.31607 4.20435 2 5 2H10C10.256 2 10.512 2.098 10.707 2.293L17.707 9.293ZM5 6C5.26522 6 5.51957 5.89464 5.70711 5.70711C5.89464 5.51957 6 5.26522 6 5C6 4.73478 5.89464 4.48043 5.70711 4.29289C5.51957 4.10536 5.26522 4 5 4C4.73478 4 4.48043 4.10536 4.29289 4.29289C4.10536 4.48043 4 4.73478 4 5C4 5.26522 4.10536 5.51957 4.29289 5.70711C4.48043 5.89464 4.73478 6 5 6Z" fill="white" />
                  </svg>
                ...
                <div class="inline-flex flex-row items-center justify-between w-full p-1 space-x-4">
                  <van-uploader class="inline-flex items-start justify-start h-full" v-model="upload_img_1" :after-read="afterRead_1" :multiple="false" :max-count="1" />
                  <van-uploader class="inline-flex items-start justify-start h-full" v-model="upload_img_2" :after-read="afterRead_2" :multiple="false" :max-count="1" />
                  <van-uploader class="inline-flex items-start justify-start h-full" v-model="upload_img_3" :after-read="afterRead_3" :multiple="false" :max-count="1" />
                </div>
                <p class="w-full text-xs text-red-500">Max 3 photos upload</p>
                <div class="flex flex-col items-start justify-center w-full space-y-4">
                  <p class="text-lg font-semibold text-gray-900 w-72">Add Description</p>
                  <textarea class="relative border-none rounded-lg h-1/4 w-80 bg-slate-200 focus:border-none" name="description" id="description" v-model="description" cols="20" rows="3"></textarea>
                </div>
              </div>
              ...
              <div class="flex flex-col items-center justify-start space-y-2">
                <div class="inline-flex items-start justify-between w-full space-x-44">
                  <p class="text-sm leading-normal">Base fee</p>
                ...
                <div v-if="service?.calculator == 'FIXED'" class="inline-flex items-start justify-between w-full space-x-44">
                  <p class="text-sm leading-normal">Service charges </p>
                ...
                <div v-if="user.wallet_balance != 0" class="inline-flex items-start justify-between w-full space-x-44">
                  <p class="text-sm leading-normal">Use wallet</p>
                  <div>
                    <InputCheckbox required class="w-4" v-model:checked="form.useWallet" id="term" />
              ...
              <div v-if="!isSchedule" class="inline-flex w-full items-start justify-start space-x-2 px-2.5">
                ...
                <button type="submit" class="flex items-center justify-center flex-1 px-5 py-3 bg-primary rounded-xl">
                  <p class="text-base font-semibold text-center text-gray-50">Request</p>
                </button>
              </div>
                  ...
                  <div class="flex flex-col w-full space-y-4">
                    ...
                    <div @click="show = true" class="inline-flex items-start justify-start w-full p-4 space-x-3 bg-gray-50 dark:bg-gray-900 border-2 rounded-xl">
                      ...
                      <div class="inline-flex flex-col items-start justify-start flex-1 space-y-1">
                        <p class="w-full text-xs font-medium leading-3 tracking-wide text-gray-500 uppercase">Date</p>
                        <p class="w-full text-base font-semibold leading-normal text-gray-900 dark:text-gray-200">Select your Date</p>
                      </div>
                      {{ form.date }}
                    </div>
                    ...
                    <div @click="showPopup" class="inline-flex items-start justify-start w-full p-4 space-x-3 bg-gray-50 dark:bg-gray-900 border-2 rounded-xl">
                      ...
                      <svg class="w-6 h-6" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20ZM12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" fill="#6F767E" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M12 6C12.5523 6 13 6.44772 13 7V11.5858L15.2071 13.7929C15.5976 14.1834 15.5976 14.8166 15.2071 15.2071C14.8166 15.5976 14.1834 15.5976 13.7929 15.2071L11.5858 13C11.2107 12.6249 11 12.1162 11 11.5858V7C11 6.44772 11.4477 6 12 6Z" fill="#6F767E" />
                      </svg>
  
                      <div class="inline-flex flex-col items-start justify-start flex-1 space-y-1">
                        <p class="w-full text-xs font-medium leading-3 tracking-wide text-gray-500 uppercase">Time</p>
                        <p class="w-full text-base font-semibold leading-normal text-gray-900 dark:text-gray-200">Select Visit Time</p>
                      </div>
                      {{ form.time }}
                    </div>
                    <van-popup v-model:show="showTime" position="bottom" :style="{ height: '40%' }" teleport="#modals">
                      <van-datetime-picker @confirm="gaetdatetime" @cancel="showPopup" v-model="form.time" type="time" title="Choose Time" />
                    </van-popup>
                  </div>
                </ClientOnly>
              ...
              <label for="type" value="type" />
                ...
                <div class="flex flex-row items-center justify-between w-full space-x-2">
                  ...
                  <button class="flex items-center content-between justify-center flex-1 w-full p-2 px-5 py-3 mt-4 text-white dark:text-gray-900 bg-primary btn-grad rounded-xl" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    <p class="text-base font-semibold text-center text-gray-50">{{ "Request" }}</p>
                  </button>
                </div>
              </div>
